...
//                         $inc: {
//                             moneySpent: highestBidAmount.amount,
//                             auctionWon: 1,
//                         },
...
export const endedAuctionCron = () => {
    cron.schedule("*/5 * * * *", async () => {
        const now = new Date();
        console.log(`[${now.toISOString()}] Cron for ended auction running...`);
        ...
        for (const auction of endedAuctions) {
            try {
                ...
                if (highestBidder) {
                    ...
                    // Send email if payment methods exist
                    if (auctioneer.paymentMethods) {
                        const subject = `Congratulations! You won the auction for ${auction.title}`;
                        const message = `Dear ${bidder.userName},\n\n...`; // Keep your email template
                        sendEmail({ email: bidder.email, subject, message });
